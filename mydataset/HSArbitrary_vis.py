"""Dataset loader for visualization/inference examples."""
...
class HSArbitraryDataVis(data.Dataset):
    """Load visualization .mat files containing a `gt` array in H x W x C format."""

    def __init__(self, image_dir, augment=False, use_3D=False, lr_size=(64, 64), scale_range=(1.0, 4.0)):
    ...
    def __getitem__(self, index):
        if self.current_scale_factor is None:
            self.current_scale_factor = float(f"{random.uniform(*self.scale_range):.1f}")

        file_index = index % len(self.image_files)
        load_dir = self.image_files[file_index]
        file_name = self.get_filename(load_dir)
        ...
        hr_patch = hr[:hr_height, :hr_width, :]
        ...
        return {"lr": lr_tensor, "hr": hr_tensor, "scale": scale_factor, "file_name": file_name}
